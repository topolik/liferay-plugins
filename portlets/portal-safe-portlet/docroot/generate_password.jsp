...
<%
	...
	char[] encodedPassword = new char[0];
	char[] plainTextPassword = new char[0];
...
<c:choose>
	<c:when test="<%= Validator.isNotNull(encodedPassword) %>">
		<aui:fieldset label="portal-configuration">
			...
			<aui:input
				disabled="true"
				inputCssClass="portalSafePassword"
				label=""
				multiple="true"
				name="portalSafePassword"
				type="textarea"
				value="<%= PropsKeys.PORTAL_SAFE_PASSWORD + StringPool.PERIOD + companyId + StringPool.EQUAL + encodedPassword %>"
				/>
		</aui:fieldset>
		...
		<aui:fieldset label="secret-key">
			...
			<div class="portlet-msg-alert">
				<liferay-ui:message key="key" />: <%= plainTextPassword %><br />
			</div>
